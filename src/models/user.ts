...
export class UserModel {
	...
	list(db: Knex, query: any, limit: any, offset: any) {
		...
		if (query) {
			...
			sql.where((builder) => {
				builder
					.whereRaw('LOWER(u.first_name) like LOWER(?)', [_query])
					.orWhereRaw('LOWER(u.last_name) like LOWER(?)', [_query]);
			});
		}

		return sql.orderBy('u.first_name').limit(limit).offset(offset);
	}
	...
	update(db: Knex, userId: any, data: any) {
		return db('users').update(data).where('user_id', userId);
	}
